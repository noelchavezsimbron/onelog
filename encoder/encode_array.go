...
func (enc *Encoder) encodeArray(v JsonArray) ([]byte, error) {
	enc.grow(200)
	enc.writeByte('[')
...
func (enc *Encoder) AddArray(v JsonArray) {
	enc.Array(v)
}
...
func (enc *Encoder) AddArrayOmitEmpty(v JsonArray) {
...
func (enc *Encoder) AddArrayNullEmpty(v JsonArray) {
...
// value must implement Marshaler
func (enc *Encoder) AddArrayKey(key string, v JsonArray) {
	enc.ArrayKey(key, v)
}
...
func (enc *Encoder) AddArrayKeyOmitEmpty(key string, v JsonArray) {
...
func (enc *Encoder) AddArrayKeyNullEmpty(key string, v JsonArray) {
...
// value must implement Marshaler
func (enc *Encoder) Array(v JsonArray) {
	if v.IsNil() {
		enc.grow(3)
...
// value must implement Marshaler
func (enc *Encoder) ArrayOmitEmpty(v JsonArray) {
	if v.IsNil() {
		return
	}
	enc.grow(4)
...
// value must implement Marshaler
func (enc *Encoder) ArrayNullEmpty(v JsonArray) {
	enc.grow(4)
	r := enc.getPreviousRune()
	if r != '[' {
		enc.writeByte(',')
...
// value must implement Marshaler
func (enc *Encoder) ArrayKey(key string, v JsonArray) {
	if enc.hasKeys {
...
// Must be called inside an object as it will encode a key.
func (enc *Encoder) ArrayKeyOmitEmpty(key string, v JsonArray) {
	if enc.hasKeys {
		if !enc.keyExists(key) {
			return
		}
...
// Must be called inside an object as it will encode a key.
func (enc *Encoder) ArrayKeyNullEmpty(key string, v JsonArray) {
	if enc.hasKeys {
		if !enc.keyExists(key) {
			return
		}
